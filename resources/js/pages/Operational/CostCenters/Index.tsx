...
export default function CostCenters({ costCenters = [], abilities }: { costCenters?: Center[]; abilities: Record<string, boolean> }) {
    ...
    return <AppLayout><Head title="Cost centers"/><PageHeader title="Cost centers" description="Organize the tenant cost center hierarchy." crumbs={[{ label: 'Cost centers' }]} action={abilities.create && <Link className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-5 py-3.5 text-sm text-white shadow-theme-xs transition hover:bg-brand-600" href="/operational/cost-centers/create">Add cost center</Link>}/>{costCenters.length ? <div className="mp-card operational-table-scroll overflow-x-auto"><table className="mp-table"><thead><tr><th>Name</th><th>Status</th><th/></tr></thead><tbody>{rows(costCenters)}</tbody></table></div> : <EmptyState title="No cost centers found">Create a cost center to start building the hierarchy.</EmptyState>}<ConfirmModal open={!!target} title={action === 'delete' ? 'Delete cost center' : 'Deactivate cost center'} confirmLabel={action === 'delete' ? 'Delete' : 'Deactivate'} onClose={() => { setTarget(null); setAction(null); }} onConfirm={execute}>Confirm {action} for {target?.name}.</ConfirmModal></AppLayout>;
}
