...
final class ProjectEconomicDatasetTest extends TestCase
{
    ...
    public function test_stage_change_reclassifies_estimate_but_actual_remains_primary_without_changing_expenses(): void
    {
        ...
        ExpenseRow::factory()->for($estimate)->create(['type' => 'estimate', 'net_amount' => '100.00']);
        ...
        $before = $calculate();
        $this->assertSame('50.00', $before['primary']);
        $this->assertSame('100.00', $before['idea']);
        $this->assertSame('150.00', $before['potential']);

        $project->forceFill(['stage' => ProjectStage::Rejected, 'lock_version' => 2])->save();
        $after = $calculate();
        $this->assertSame('50.00', $after['primary']);
        $this->assertSame('100.00', $after['excluded']);
        $this->assertSame('50.00', $after['potential']);
