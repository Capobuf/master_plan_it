...
- Do not rely on any spec/import pipeline. The source of truth is the exported files under `master_plan_it/master_plan_it/`.
- Avoid creating duplicate metadata paths outside the canonical module folder.

## Applying changes
- Edit canonical metadata and controllers in `master_plan_it/master_plan_it/`.
