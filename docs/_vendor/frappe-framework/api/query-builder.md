...
### frappe.qb.Doctype(name\*of\*table)

Returns a PyPika table object which can be used elsewhere. It will automatically add 'tab' if necessary.

### frappe.qb.Table(name\*of\*table)

Does the same thing as `frappe.qb.DocType` but will not append 'tab'. It's intended to be used with '\*\*Auth' like tables.

> Note: You should only use this if you know what you are doing.

### frappe.qb.Field(name\*of\*coloum)
