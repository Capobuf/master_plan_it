...
If there are no redirects for incoming requests path resolver tries to resolve the route to get the final endpoint based on rules from [website\*routing\*rules hook](hooks.md#website-route-rules) and dynamic route set in documents of DocType with `has_web_view` enabled.

### Renderer Selection

Once the final endpoint is obtained it is passed through all available [Page Renderers](routing-and-rendering.md#page-renderer) to check which page renderer can render the given path. A first page renderer to return `True` for `can_render` request will be used to render the path.
...
**Note:** Custom page renderers get priority and it's `can_render` method will be called before [Standard Page Renderers](routing-and-rendering.md#page-renderer).
