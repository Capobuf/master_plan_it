...
| Expenses: current register | `expense.view` | `ExpenseRegisterQuery` | `/api/v1/expenses` | GET | `year`, `kind`, `page`, `per_page` | paginated `ExpenseRegisterResource` plus exact filtered totals | 401, 403, 404, 422 | selected tenant only; `kind=plafond` supports the editor lookup |
...
| Contract register search (`q`) | `PLANNED` | The current `ContractListQuery` does not apply a search term. The launch UI omits the control and the OpenAPI list operation documents only working pagination parameters. |
| Expense register search and cost-center filters | `PLANNED` | The current register query supports year and kind only. The launch UI omits `q` and cost-center controls rather than simulating authoritative filtering in the client. |
