...
| Q-039 | Product language | MEDIUM | Is launch Italian-only? | ANSWERED / AMENDED 2026-08-09 — yes for the current launch UI; this explicitly supersedes Q-013 only for current presentation. Budget, Report, Tenant, Master Plan IT, proper names and non-prose technical identifiers are the only English exceptions. No i18n framework or selector is introduced and `language_code` remains in the domain/API for future use. |
