...
| R-BUD-001 | Current Budget is one rolling tenant/year calculation from current Expense rows; manual no-project data requires no Feature 004 generation | Q-036; PD-BUD-001; ADR-035 | FR-005-001/011; INV-ECO-001; INV-BUD-003 | T003-008, T003-011, T005-001–T005-008, T005-026 | `EconomicDatasetQueryTest`, `CurrentBudgetParityTest`, `ExpenseBudgetVerticalSliceTest` |
...
| R-UI-001 | Tenant-facing Expense/Budget/reporting and later operational project/contract UI uses the existing auth/session/context/authorization/domain stack through Blade/Livewire/Alpine/Tailwind/mandatory Preline; Chart.js receives server-calculated values and Livewire DOM changes explicitly reinitialize/clean JavaScript-owned behavior | ADR-035; FR-001-022; development/test contract | FR-001-022; INV-PLT-008; NFR-001-A11Y-01; NFR-001-COMPAT-01 | T001-028, T001-029, T003-009, T003-012, T004-006, T004-009, T005-007, T005-008, T005-026 | `FrontendStackContractTest`, `OperationalShellTest`, `OperationalShellSmokeTest`, Livewire operational page tests, `ExpenseBudgetVerticalSliceTest` |
...
| R-MD-REV-001 | Vendor/CostCenter use allowlisted Overtrue v6 SNAPSHOT history with package actor attribution; application revision batches own UUID correlation, the closed lifecycle vocabulary and ordered links from RES-002-011, and only domain restore Actions may create a validated new current revision | C-05/C-12; PD-REV-001; RES-002-004/010/011 | FR-002-012; INV-MD-REV-001 | T002-004–T002-006, T002-010–T002-015, T002-017 | `VersioningPackageSmokeTest`, `RevisionBatchSchemaTest`, `RevisionBatchIntegrationTest`, cost-center/vendor revision tests |
...
| R-PWD-001 | No public password recovery; Administrator reset, self-change and emergency interactive reset invalidate every affected database session and rotate the remember token without exposing secrets, while ordinary logout preserves other sessions and that token | Q-026; Feature 001/007 acceptance; C-05/C-07; RES-001-014/020 | FR-001-014–017; FR-007-018; FR-007-019; INV-PLT-006 | T001-008, T001-012–T001-016 | no-recovery-route, password administration, self-change and console reset tests |
