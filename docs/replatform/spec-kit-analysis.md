...
| Constitution reflects approved amendments | PASS | Constitution 3.0.1 replaces fixed-role and immutable-Actual rules, adds revision/budget-version/generation principles, explicit output scopes, and configurable audit retention |
...
| Audit exported at launch | NO | Q-020 and tenant-portability contract exclude audit export |
| Audit retention fixed in code | NO | Constitution 3.0.1 defines an Administrator-only global setting with 24-month default |
| Export scope changes silently | NO | Q-019 requires explicit filtered or complete selected report/year scope and one tenant |
...
|---|---|---|---|
| AN-CL-001 | HIGH | Feature plans and tasks predate Constitution 3.0.1 and contain fixed roles, immutable Actual, replacement-state, implicit output scope, fixed audit retention, and missing versioning/portability/generation work. | Run `/speckit.plan`, then `/speckit.tasks`; implementation is blocked until regenerated. |
...
| AN-CL-004 | MEDIUM | Feature 004 and 005 detailed screen/query/task contracts do not yet define final physical symbols and dataset keys for generation history, budget comparisons, and filtered/complete output scopes. | Resolve in `/speckit.plan`, then produce exact tasks. |
...
| AN-CL-006 | MEDIUM | Complete tenant portability scope for retained operational revisions and notifications requires a technical retention/format decision; audit is already excluded. | Resolve without changing product exclusions or portability boundary. |
| AN-CL-007 | MEDIUM | PR #1 and PR #2 were authored against the prior product model. | Merge this clarification PR first, then rebase and reconcile both branches, especially economic-kernel, accounting, Feature 001, and task assumptions. |
...
- `/speckit.plan`: NEXT VALID COMMAND after merge and branch reconciliation
