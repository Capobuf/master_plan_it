...
| Q-039 | `AMENDED 2026-08-09 — SUPERSEDES Q-013 FOR THE CURRENT LAUNCH UI` — the current Master Plan IT user interface is Italian. Only the common terms Budget, Report and Tenant, the Master Plan IT brand, proper names and technical identifiers not presented as ordinary user copy may remain in English. This launch decision does not add an i18n framework or language selector and does not remove `language_code` from the domain/API, which remains available for future product evolution. Rationale: one consistent, reviewable launch language avoids a partially localized interface while preserving the tenant data contract. | All current frontend copy, browser routes, labels, dates and presentation formatting; no API endpoint, persistence, tenancy or future localization-contract change. |
