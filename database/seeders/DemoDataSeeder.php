...
final class DemoDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        Model::unguarded(fn () => DB::transaction(function () use ($administrator): void {
            $tenant = Tenant::query()->updateOrCreate(['code' => 'demo'], ['name' => 'Azienda Demo S.r.l.', 'state' => TenantState::Active, 'currency_code' => 'EUR', 'language_code' => 'it', 'timezone' => 'Europe/Rome', 'default_vat_rate' => '22.00', 'budget_basis' => BudgetBasis::Net, 'created_by_user_id' => $administrator->getKey()]);
            ...
            for ($i = 1; $i <= 24; $i++) {
                ...
                if ($i % 6 === 0) {
                    $this->row($tenant, $expense, $administrator, 2, $vendors[($i + 1) % count($vendors)]->id, ExpenseType::Quote, 'Additional demo row', '250.00', sprintf('%04d-%02d-20', $label, ($i % 12) + 1), null, false);
                }
            }
