...
final class ContractController extends Controller
{
    ...
    private function contractData(Request $request, bool $update): SaveContractData
    {
        ...
        $rules = [
            ...
            'terms.*.quantity' => ['nullable', 'string', 'regex:/^-?\d+(?:\.\d{1,2})?$/D'],
            'terms.*.unit_price' => ['nullable', 'string', 'regex:/^-?\d+(?:\.\d{1,2})?$/D'],
            'terms.*.entered_amount' => ['required', 'string', 'regex:/^-?\d+(?:\.\d{1,2})?$/D'],
            'terms.*.amount_includes_vat' => ['required', 'boolean'],
            'terms.*.vat_rate' => ['nullable', 'string', 'regex:/^\d+(?:\.\d{1,2})?$/D'],
