...
final class TenantSettingsController extends Controller
{
    ...
    {
        $allowed = ['name', 'timezone', 'default_vat_rate', 'budget_basis', 'deletion_reason_required', 'lock_version'];
        ...
        return TenantSettingsResource::make($action->execute(
            $this->actor($request), $this->tenantContext($request), $validated,
            $lockVersion, $this->correlationId($request),
        ));
    }
}
