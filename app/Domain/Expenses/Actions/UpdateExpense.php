...
final class UpdateExpense
{
    ...
    public function execute(User $actor, TenantContext $context, Expense $target, SaveExpenseData $data, array $rows, string $correlationId): Expense
    {
        $this->expensePolicy($context)->update($actor, $target)->authorize();
        [$actor, $tenant] = $this->persistedContext($actor, $context);
        return DB::transaction(function () use ($actor, $context, $correlationId, $data, $rows, $target, $tenant): Expense {
            ...
            if($sourceRowIds!==[]){$submittedIds=array_values(array_filter(array_map(fn($row)=>$row->id,$rows),fn($id)=>$id!==null));if((int)$expense->planning_year_id!==$data->planningYearId||(int)$expense->contract_id!==(int)$data->contractId||array_diff($sourceRowIds,$submittedIds)!==[]){throw new DomainException('TENANT_RELATION_MISMATCH');}}
