    ...
    public function execute(User $actor, TenantContext $context, Contract $contract, string $sourceKey, string $correlationId): void
    {
        ...
        DB::transaction(function () use ($actor, $tenant, $contract, $sourceKey, $correlationId): void {
            ...
            if (! $exception instanceof ContractGenerationException) {
                throw new DomainException('OCCURRENCE_NOT_SUPPRESSED');
            }$exception->delete();
            app(AuditRecorder::class)->record('contract.occurrence-resumed', $correlationId, new AuditProperties(['source_key' => $sourceKey]), $actor, (int) $tenant->getKey(), $contract);
        });
    }
}
