...
final class EconomicEngine
{
    ...
    public function calculate(EconomicDataset $dataset): array
    {
        ...
        $byCostCenter = [];
        $plafondGroups=[];$consumedGroups=[];$fundedLines=[];
        foreach ($dataset->lines as $line) {
            ...
            if ($line->fundedPlafondExpenseId === null) {$amounts['officialCurrentPosition'] = bcadd($amounts['officialCurrentPosition'], $official, 2);}
            ...
            foreach ($line->fundedPlafondExpenseId === null ? $this->monthly($dataset, $line->spendDate, $line->periodStart, $line->periodEnd, $line->distribution, $official) : [] as $key => $value) {
                if (isset($monthly[$key])) { $monthly[$key] = bcadd($monthly[$key], $value, 2); }
            }
        }
        $amounts['plafondResidual']='0.00';$amounts['plafondOverrun']='0.00';
        foreach(array_unique([...array_keys($plafondGroups),...array_keys($consumedGroups)]) as $group){$allocation=$plafondGroups[$group]??['official'=>'0.00','net'=>'0.00','vat'=>'0.00','gross'=>'0.00'];$consumption=$consumedGroups[$group]??['official'=>'0.00','net'=>'0.00','vat'=>'0.00','gross'=>'0.00'];$difference=bcsub($allocation['official'],$consumption['official'],2);if(bccomp($difference,'0',2)>=0){$amounts['plafondResidual']=bcadd($amounts['plafondResidual'],$difference,2);}else{$overrun=ltrim($difference,'-');$amounts['plafondOverrun']=bcadd($amounts['plafondOverrun'],$overrun,2);$amounts['officialCurrentPosition']=bcadd($amounts['officialCurrentPosition'],$overrun,2);$line=$fundedLines[$group]??null;if($line!==null){$byCostCenter[$line->costCenterName]=bcadd($byCostCenter[$line->costCenterName]??'0.00',$overrun,2);foreach($this->monthly($dataset,$line->spendDate,$line->periodStart,$line->periodEnd,$line->distribution,$overrun) as $key=>$value){if(isset($monthly[$key])){$monthly[$key]=bcadd($monthly[$key],$value,2);}}}}foreach(['net','vat','gross'] as $dimension){$covered=bccomp($allocation[$dimension],$consumption[$dimension],2)<=0?$allocation[$dimension]:$consumption[$dimension];$amounts[$dimension]=bcsub($amounts[$dimension],$covered,2);}}
    ...
    {
        if ($spend !== null) { return [substr($spend, 0, 7) => $amount]; }
        if ($start === null || $end === null || $distribution === null) { return []; }
