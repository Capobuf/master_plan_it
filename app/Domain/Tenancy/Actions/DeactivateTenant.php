...
final class DeactivateTenant
{
    ...
    public function execute(
        ...
        if (! $this->platformAdministrator->allows($actor, 'platform.tenants.deactivate')) {
            throw new AuthorizationException('PERMISSION_DENIED');
        }

        return DB::transaction(function () use ($actor, $confirmationToken, $correlationId, $expectedLockVersion, $target): Tenant {
            ...
            if ($confirmationToken !== $tenant->code) {
                throw new DomainException('DESTRUCTIVE_CONFIRMATION_REQUIRED');
            }
            ...
            $updated = Tenant::query()
                ...
                ->where('state', TenantState::Active->value)
