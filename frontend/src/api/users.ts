...
export interface TenantUser { id: number; name: string; email: string; active: boolean; tenant_id?: number | null; roles?: Array<{ id: number; name: string }>; role_ids?: number[]; }
...
export async function listUserRoleOptions(): Promise<PaginatedData<{id:number;name:string}>> { return (await apiClient.get<PaginatedData<{id:number;name:string}>>("/api/v1/user-role-options", {params:{per_page:100}})).data; }
