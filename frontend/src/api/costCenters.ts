...
export async function deleteCostCenter(id: number, lock_version: number): Promise<void> { await apiClient.delete(`/api/v1/cost-centers/${id}`, { data: { lock_version } }); }
...
export async function restoreCostCenter(id: number, revisionId: number, lock_version: number): Promise<CostCenter> { return (await apiClient.post<DataEnvelope<CostCenter>>(`/api/v1/cost-centers/${id}/history/${revisionId}/restore`, { lock_version })).data.data; }
