...
export default function BudgetHome() {
  ...
  const [state, setState] = useState<{ tenantId: number; data: AnnualBudget | null; error: ApiError | null } | null>(null);
  ...
  useEffect(() => {
    ...
    void getBudget({ planning_year_id: selectedPlanningYearId, ...(asOf ? { as_of: asOf } : {}) })
      .then((data) => { if (active) setState({ tenantId, data, error: null }); })
      .catch((error: unknown) => { if (active) setState({ tenantId, data: null, error: ApiError.from(error) }); });
    return () => { active = false; };
  ...
  else if (current.data) content = <BudgetView key={`${current.data.budget.lock_version}:${current.data.mode}`} dataset={current.data} asOf={asOf} canApprove={hasAbility("expense.update")} canClose={hasAbility("planning-year.update")} onAsOfChange={setAsOf} onChange={(data) => setState({ tenantId, data, error: null })} />;
