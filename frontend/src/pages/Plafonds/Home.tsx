...
import { listAllPlafonds, type PlafondSummary } from "../../api/plafonds";
...
export default function PlafondsHome() {
  ...
  const [data, setData] = useState<PlafondSummary[] | null>(null); const [error, setError] = useState<ApiError | null>(null);
  useEffect(() => { if (!hasAbility("expense.view") || selectedPlanningYearId === null) return; let active = true; setData(null); setError(null); void listAllPlafonds({ planning_year_id: selectedPlanningYearId }).then((response) => { if (active) setData(response); }).catch((cause: unknown) => { if (active) setError(ApiError.from(cause)); }); return () => { active = false; }; }, [hasAbility, selectedPlanningYearId]);
  ...
  else if (data.length === 0) body = <Alert variant="info" title="Nessun Plafond" message="Non ci sono Plafond per l’anno selezionato." />;
  else body = <PlafondRegister plafonds={data} />;
