            ...
            {items.map((item) => (
              <TableRow key={item.expense_id}>
                <TableCell className="py-2 pr-2 sm:py-2.5 sm:pr-4">
                  <Link to={routes.spesa(item.expense_id)} className="line-clamp-2 text-sm font-medium leading-5 text-gray-800 hover:text-brand-500 dark:text-white/90 dark:hover:text-brand-400 sm:max-w-[220px]">{item.title}</Link>
                  <span className="mt-0.5 block text-theme-xs text-gray-400 dark:text-gray-500">{formatDateTime(item.updated_at)}</span>
                </TableCell>
                <TableCell className="hidden max-w-[150px] truncate py-2.5 pr-4 text-sm text-gray-600 dark:text-gray-300 md:table-cell">{item.cost_center_name}</TableCell>
                <TableCell className="hidden max-w-[150px] truncate py-2.5 pr-4 text-sm text-gray-600 dark:text-gray-300 xl:table-cell">{item.vendor_summary ?? "—"}</TableCell>
                <TableCell className="hidden max-w-[150px] truncate py-2.5 pr-4 text-sm text-gray-600 dark:text-gray-300 2xl:table-cell">{item.project_title ?? "Senza progetto"}</TableCell>
                <TableCell className="hidden whitespace-nowrap py-2.5 pr-4 text-end text-sm text-gray-700 dark:text-gray-300 sm:table-cell">{formatMoney(item.totals.current_planning.official, currency)}</TableCell>
                <TableCell className="whitespace-nowrap py-2 pr-2 text-end text-xs font-medium text-gray-800 dark:text-white/90 sm:py-2.5 sm:pr-4 sm:text-sm">{formatMoney(item.totals.actual.official, currency)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
