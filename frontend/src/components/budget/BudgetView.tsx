...
export default function BudgetView({ dataset, asOf, tenantTimezone, canApprove, canClose, onAsOfChange, onChange }: { dataset: AnnualBudget; asOf: string; tenantTimezone: string; canApprove: boolean; canClose: boolean; onAsOfChange: (value: string) => void; onChange: (value: AnnualBudget) => void }) {
  ...
  return <div className="space-y-6">
    ...
    <ComponentCard title="Controlli annuali"><div className="flex flex-wrap gap-6 text-sm"><span>Sforamento Plafond: <strong>{formatMoney(summary.plafond_overrun ?? "0.00", summary.currency ?? dataset.currency ?? "EUR")}</strong></span><span>Effettivi senza approvato: <strong>{summary.unapproved_actual_expenses ?? 0}</strong></span></div></ComponentCard>
