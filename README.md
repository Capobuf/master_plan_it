...
Laravel 13 modular monolith on PHP 8.3/MySQL 8.4, Sail verification, Inertia 3 + React 19 + TailAdmin-inspired Tailwind CSS 4 for the unified application UI, Chart.js, explicit one-database tenancy, configurable tenant RBAC, operational snapshot revisions, one current Expense source, one shared economic kernel, rolling Budget, later controlled contract generation/BudgetVersion/output, staged migration/portability, verified whole-installation backup and immutable release artifact. The first planned usable slice is manual Expense → current economic dataset → current Budget.
