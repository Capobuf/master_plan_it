...
Purpose: column schema, validation, streaming, authorization and error report.
...
This interface inherits the exact ability/context/ownership/state decision procedure from `authorization.md`. Editor and Viewer names are seed-template examples only and never select behavior. An export requires the corresponding same-tenant `planning-year.view`, `cost-center.view`, or `vendor.view` ability; import requires protected `platform.migration.run`. Missing ability/context, inactive state, or foreign identity fails closed without existence disclosure.
...
Read the local plan and data model. Implement exactly column schema, validation, streaming, authorization and error report. Do not reuse this file as a generic abstraction for other domains; shared behavior belongs only in an explicitly listed shared helper.
## Tenant clauses
