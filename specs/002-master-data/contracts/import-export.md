...
- Import is Administrator-only and requires an explicit target tenant.
- Every imported key, parent, and reference is collision-checked inside that tenant and cannot resolve to another tenant.
- Editor and Viewer may export only same-tenant data according to their read permissions.
