...
Purpose: persistence required by Master data. Exact columns and migration order are defined in the plan file map. Delete behavior defaults to `restrict`; soft delete is used only for user-authored master/business records that must remain referenceable.
