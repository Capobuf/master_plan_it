...
| Seeded Editor template | one tenant | ordinary create/update/delete/version/restore business permissions, scenarios, attachments, reports, and approved generation controls | no protected platform operations |
...
Given a newly created tenant, when default access is seeded, then Editor and Viewer templates match Q-005 through Q-009 while remaining editable templates rather than code branches.
...
| FR-007-005 | Editor and Viewer shall be seeded tenant role templates matching approved initial permission sets. | AC-007-05 |
