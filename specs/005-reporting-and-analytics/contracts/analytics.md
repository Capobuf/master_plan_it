...
Purpose: metrics, formulas, filters and typed payloads.
...
This interface inherits the exact ability/context/ownership/state decision procedure from `authorization.md`. Analytics require `report.view` plus a valid one-tenant scope; version/scenario comparison additionally requires its distinct compare ability. Editor and Viewer names are seed-template examples only and never select behavior. Missing ability/context, inactive state, or foreign identity fails closed without existence disclosure.
...
Read the local plan and data model. Implement exactly metrics, formulas, filters and typed payloads. Do not reuse this file as a generic abstraction for other domains; shared behavior belongs only in an explicitly listed shared helper.
## Tenant clauses

Economic analytics are tenant-scoped. Cross-tenant comparisons and currency conversion are not approved scope.
