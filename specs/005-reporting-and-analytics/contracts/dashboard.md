...
The tenant current-Budget surface is a Blade/TailAdmin page in the shared application layout. It renders minimum filters, KPI, equivalent table, one useful ApexCharts chart and loading/empty/error states from the same server dataset. TailAdmin/Alpine owns local chart lifecycle per INV-PLT-008, and no browser code calculates economics.
