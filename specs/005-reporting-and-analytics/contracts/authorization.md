...
- Current Budget/report reads, print, filtered export, complete export, and reference selection require the distinct matching `budget.*` or `report.*` ability; output never widens the authorized tenant/scope.
- BudgetVersion view/create/update-draft/publish/compare/reference operations require their distinct `budget-version.*` ability; no permission permits published mutation/deletion.
- Scenario view/create/update/archive/delete/compare operations require their distinct `scenario.*` ability; tenant-role names do not determine scenario access.
- The global operational overview requires the protected global Administrator boundary; no tenant ability grants that surface.
- The global overview contains no cross-tenant economic aggregates.
