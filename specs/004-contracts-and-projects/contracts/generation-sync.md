# Contract — Generation Sync

Feature: `004-contracts-and-projects`  
Purpose: source key, expected row, append-missing algorithm, idempotency and locks.
...
Read the local plan and data model. Implement exactly source key, expected row, append-missing algorithm, idempotency and locks. Do not reuse this file as a generic abstraction for other domains; shared behavior belongs only in an explicitly listed shared helper.
## Tenant isolation clauses

Generation receives explicit tenant ownership from the contract. It may create or link only expenses and source identities in that tenant. Missing or mismatched tenant context fails closed before a row is created.
