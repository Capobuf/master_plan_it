...
Purpose: release ZIP, shared hosting and Docker profiles, health check, rollback.
...
Read the local plan and data model. Implement exactly release ZIP, shared hosting and Docker profiles, health check, rollback. Do not reuse this file as a generic abstraction for other domains; shared behavior belongs only in an explicitly listed shared helper.
## Tenant clauses

Deployment hosts one multi-tenant Laravel application. No custom tenant domain or separate tenant database is required by the approved product specification.
